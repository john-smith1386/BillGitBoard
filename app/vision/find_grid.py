...
def _deduplicate(blobs: Iterable[SquareBlob], scale: float) -> list[SquareBlob]:
    """Keep the best blob in each `radius` neighbourhood, highest quality first.

    Selected blobs are indexed into a grid of `radius`-sized buckets, so each
    candidate compares against the nine buckets that can hold a neighbour within
    the radius rather than against every blob kept so far. That is the same
    decision the naive scan makes, but it keeps a dense field of small squares -
    a checkerboard costs a few hundred bytes to encode - from turning this into
    quadratic work on an unauthenticated request.
    """
    radius = max(1.25, scale * 0.28)
    selected: list[SquareBlob] = []
    buckets: dict[tuple[int, int], list[SquareBlob]] = {}

    for blob in sorted(blobs, key=lambda item: item.quality, reverse=True):
        key_x = int(blob.x // radius)
        key_y = int(blob.y // radius)
        if any(
            math.hypot(blob.x - other.x, blob.y - other.y) <= radius
        ...
        buckets.setdefault((key_x, key_y), []).append(blob)
    return selected
